...
**Format discipline (v0.2.0+, updated v0.3.0):** the fields list MUST be rendered as a **bullet list with bold field names**, NOT as a code block. Code blocks do not word-wrap and overflow narrow chat surfaces horizontally; bullet lists wrap at any container width.
...
- **Bullet list, not code block.** Code blocks overflow narrow UIs horizontally.
- **Bold the field name** (`**Account.Name:**`) — makes the eye scan column 1 easily.
- **Parenthetical notes in italics** `*(note)*` on the same line — wraps gracefully.
- **Backticks for IDs/keys** that benefit from monospace (`001gK...`, picklist values) — inline `code` style still works inside bullets.
- **NEVER use pipe-syntax tables for fields data.** Small fixed-width reference tables (like the rubric mapping at the top of this skill) stay as tables; record data uses bullets.
